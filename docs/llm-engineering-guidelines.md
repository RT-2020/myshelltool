...
| 形态 | 定义 | 本项目实证（均已修复） |
|---|---|---|
| **A 环境假设** | 假设外部布局/工具链/平台长某样 | 前端猜家目录 `/home/<username>`——root 的家在 `/root`，连接 117 服务器直接 `No such file`；`find -printf` 假设 GNU coreutils——BusyBox/Alpine/BSD 上不存在；`df -B1` 同为 GNU-only；本地系统目录黑名单硬编码 `c:\windows`——系统装 D 盘即失守；快速连接用 `slugify(host)` 当资产 id——同主机不同用户名互相覆盖且沿用错误凭据。**第二轮追加**：命令串写 `LC_ALL=C df -h` 假设远端 shell 是 POSIX sh——FreeBSD 的 root 默认 csh/tcsh，`VAR=值 命令` 不是赋值语法，整条 `df` 不执行（改用 `env LC_ALL=C`）；`MYSHELLTOOL_DATA_DIR` 假设「环境变量存在即非空且绝对」——空值/相对值让配置落到进程 CWD 并随启动方式漂移 |
| **B 格式猜测** | 按固定格式/字符串嗅探解析外部数据 | `df` 输出按英文表头 `"Filesystem"` 切分且无 `LC_ALL=C`——中文 locale 下磁盘统计静默归零；/proc/diskstats 父设备与分区（sda+sda1）双计数，IO 虚高一倍；OSC 7 路径只编码空格却全量 `decodeURIComponent`——目录名含 `%` 即错乱。**第二轮追加**：管道段后紧跟 `echo rc_top=$?` 被当作「该段真实退出码」——它其实是 `head` 的（恒 0），无 procps 的容器上把「top 不存在」伪装成成功；远端文件名经 `from_utf8_lossy` 解码（GBK/latin-1 名变 U+FFFD 后再也回不去，rename/删除/下载全 `No such file`）；`/proc/diskstats` 只按「整盘↔分区」去重——LVM（dm-0 与 PV 整盘）与软 RAID（md0 与成员盘）记同一份 IO，磁盘读写虚高 2-3 倍 |
| **C 静默兜底掩盖错误** | 把「查不到/出错」折叠成默认值继续跑 | 上传循环信任目录列表时刻的 size——增长的文件（日志/导出）静默截断且标记成功；上传前 stat 失败当「目标不存在」——覆盖确认被绕过；资源监控解析失败发全 0 假快照。**第二轮追加（全部是 fail-open 方向）**：读 mcp-config.json 失败或含未知档位 → 回落 **Minimal（零审批档）**，用户显式设的 Strict 静默降级；执行日志「读失败」被当「空日志」，下一次 append 把整份审计历史覆盖成 1 条；凭据同步「读不到就跳过」却仍报「已推送」，Gist 备份静默缺项 |
...
| **E 形状猜测（形态 B 的升级版，也最隐蔽）** | 用一个**正则/前缀/子串**去认一个**结构化对象**（命令串、路径、设备树、身份），被等价写法绕过 | **命令层**：审批白名单整串前缀匹配——`df -h; cat /etc/shadow` 以 `df` 开头即判 Safe，Strict 档也免人工确认（改为按 shell 词法分段、逐段判定）；**路径层**：敏感文件判定按 `/.ssh/` 子串——`sftp_list(path=".")` 返回的 `./.ssh/id_rsa` 归一后无前导斜杠，**整类相对路径漏判**，私钥可免审批读取（违反 §8 凭据红线）；系统目录黑名单只认「盘符 / `\\server\share`」字面形态——`\\?\UNC\...` 与 `\\?\Volume{...}\` 是同义写法，直接绕过（可递归删除网络共享目录） |

> **形态 E 的教训**：前四种猜的是「值」，这一种猜的是「形状」——**正则只认字面，而外部世界允许等价写法**。凡是「用字符串形态做安全/正确性判定」的地方都要追问：同一语义还有没有别的写法？能不能换成结构化判定（按 shell 词法分段、按 `Path::components()` 判段、按协议解析）？**判定用的形状与实际使用的形状是不是同一个**？
...
✅ **安全判定失败必须 fail-closed（更严的一档）**：配置/状态读不出来、解析失败、遇到未知枚举值时，**不允许回落到更宽松的行为**。区分两种默认值：「文件不存在 = 用户从未表达偏好」可回落到产品默认档；「文件存在但不可信 = 用户表达过偏好而我们读不到」必须取**更严**的一档 + log + 隔离保留证据（本项目事故：拦截等级读盘失败回落 Minimal，把用户设的 Strict 静默变成零审批放行）。
✅ **判定形状必须与使用形状一致（形态 E）**：用字符串/正则做安全或正确性判定前，先确认「判定所依赖的形状」与「后续真正使用的形状」是同一个——命令串要按 shell 词法分段后再逐段判定（不能只看前缀）；路径要按组件判定并先归一（不能靠子串）；协议数据要按字段判定（不能按整串嗅探）。同一语义的等价写法（`./x` 与 `/x`、`A;B` 与 `A`、`\\srv\share` 与 `\\?\UNC\srv\share`、`dm-0` 与 `sda`）都要覆盖到，覆盖不了就换结构化判定。
...
| `no-csh-hostile-env-prefix`（v2.6） | `VAR=值 命令` / `export VAR=值` 形态的命令串 | FreeBSD（root 默认 csh）上 `df` 整条不执行，磁盘工具取不到数据 |
| `no-pipeline-rc-echo`（v2.6） | 管道段后紧跟 `echo rc_*=$?` | rc 是管道末端命令的码（恒 0），把「命令不存在」伪装成该段成功 |
| `no-fake-type-cast-call`（v2.6） | `(x as unknown as { m(): ... }).m()` 断言后调用 | 调用不存在的方法必抛 TypeError → 「刷新列表」永不生效（TS 迁移后新增形态） |
| `no-empty-catch-block`（v2.6） | 空 `catch {}`（不带理由注释） | 错误被折叠成「什么也没发生」（跨窗口 ACK / 传输 / 持久化失败都长在这里） |
| `no-drive-root-fallback`（v2.6） | `PathBuf::from("C:\\...")` 当回退/临时目录 | 系统盘非 C: 或根目录不可写 → 建目录失败被吞 → 后续在无关处 panic |
| `no-env-path-without-absolute-check`（v2.6） | `env::var("...DIR/PATH/HOME/TEMP...")` 直接当路径根 | 空值/相对值 → 配置落到进程 CWD，重启后静默回落默认档 |
...
- **review 验收按五种形态扫**：A 环境假设 / B 格式猜测 / C 静默兜底 / D 时序猜测 / E 形状猜测——尤其 C 与 E（门禁最难覆盖）：错误路径是否把失败折叠成了默认值、且折叠方向是**更宽松**的？安全/正确性判定用的字符串形态，是否存在等价写法绕过（相对路径、`;` 组合、verbatim 前缀、栈式设备）？
- **失败方向审查（每个 catch / `unwrap_or*` / `.ok()` 都过一遍）**：失败后系统是更严还是更松？涉及审批、凭据、拦截等级、数据覆盖的路径**只允许更严**。
- **并行 Agent 改同一文件时的合并纪律**（本项目真实踩坑）：多个 agent 同时改一个文件，各自跑 `type-check` 都通过，合起来却因「return 块一处陈旧引用」连环报错 20 条。收尾必须**在全部 agent 停止后**重跑一次全量门禁（`npm run build`），并以合并后的工作树为准复核 diff。
...
- [ ] **形状是否被猜（形态 E）**：凡是用字符串/正则/前缀做安全或正确性判定处，等价写法能否绕过？（`df; rm` 前缀、`./x` vs `/x`、`\\?\UNC\`、`dm-0` vs `sda`）判定形状与使用形状是否一致？能否改结构化判定？（§7）
- [ ] **失败方向**：每个 catch / `unwrap_or*` / `.ok()` 失败后是更严还是更松？审批、凭据、拦截等级、数据覆盖路径**只允许更严**（fail-closed）。（§7）
