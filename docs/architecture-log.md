...
| `src/stores/sync.ts` | 489 | store .js (soft 300 / hard 500) | **98% of hard**锛?6-29 v1.6 +autoSync/remoteUpdates 鐘舵€?+ 4 action + attachWorkbench锛?23鈫?39锛?2026-09-18 同步结果 toast 化 + 自动同步成败播报后实测 489，贴线：下次改动前应把自动同步串行队列（enqueueAutoPush 段）外迁 lib |
