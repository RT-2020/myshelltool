...
impl<'a> serde::Serializer for &'a mut Serializer {
    ...
    fn serialize_str(self, v: &str) -> Result<Self::Ok, Self::Error> {
        // 【myshelltool fork 补丁】与 buf.rs::decode_lossy_reversible 配对：
        // 读入时非法字节被编为 U+E000+b，此处解码回原始字节，保证
        // 「列目录拿到的名字 → remove/rename/open 回发」字节级往返一致。
        let bytes = crate::buf::encode_back_to_wire(v);
