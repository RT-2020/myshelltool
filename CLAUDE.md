...
>
> 本文件仅补充 Claude Code 工作流相关的提示；所有技术栈、目录结构、约定、命令、IPC 契约、数据模型、安全红线**一律以 `AGENTS.md` 及其指向的 `docs/` 参考层为准**。请先读 `AGENTS.md`（常驻指令），再按任务触及面读它顶部指针表里的 docs 文件。

## 工作流

1. **先读 `AGENTS.md`**，再开始任何改动；任务涉及目录归属 / IPC 契约 / 数据模型 / 某模块历史坑时，按其顶部指针表读对应 `docs/` 文件（`目录结构详解` / `IPC契约与数据模型` / `已知边界与修复史`）。
...
- **跨 store 逻辑**：加子 store action + `workbench.ts` re-export，用 `attachWorkbench` lazy bridge，**禁止循环 import**。`workbench.ts` 暴露子 store 响应式 state 必须用 `computed()` 包裹。
...
- 不要在派生 agent 文件（本文件、`copilot-instructions.md`）里写与 `AGENTS.md` 冲突或漂移的内容——改架构时改 `AGENTS.md` 这个唯一信息源。
...
- **文件不过大**：Vue/store ≤500 行、Rust ≤800 行；当前超标清单以 `docs/architecture-log.md` 的 Baseline snapshot 为唯一信息源，往里加功能前先评估拆分。
