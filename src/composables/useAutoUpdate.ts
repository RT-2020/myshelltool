...
import { isTauriRuntime, invokeBackend } from '../services/backend';

// Tauri v2 的 Update 类型无 relaunch（重启移到后端 app_relaunch 命令），但下方
// fallback 分支运行时探测旧实例的 relaunch 方法——类型层保留为可选成员。
type PendingUpdate = Update & { relaunch?: () => Promise<void> };

// announce 与 workbench.announce 同形：message + 可选 level。
type AnnounceFn = (message: string, options?: { level?: string }) => void;
...
export function useAutoUpdate({ announce }: { announce: AnnounceFn }) {
  ...
  function setState(next: string, message?: string, opts?: { level?: string }) {
    state.value = next;
