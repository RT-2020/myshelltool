...
/** 可拖拽区域：左侧栏 / 右侧栏 / 中间行（终端-文件分界）。 */
export type ResizeRegion = 'sidebar' | 'right' | 'center-row';
...
// localStorage 持久化的布局 JSON（字段逐个 typeof 收窄后再用，边界不信任）。
interface StoredLayout {
  sidebarW?: unknown;
  rightW?: unknown;
  centerTopH?: unknown;
  terminalRatio?: unknown;
}
...
const DEFAULTS = {
  ...
  rightW: 280,
  terminalRatio: 0.55,
  centerTopH: null as number | null // 兼容旧存储：null = 按 terminalRatio 计算
};

function readStoredLayout(storageKey: string = LAYOUT_STORAGE_KEY): StoredLayout | null {
  try {
    ...
    // JSON.parse 边界：结果收窄为 object 才视为布局数据
 ...
 * 折叠态下对应列的拖拽跳过。
 */
function isRegionCollapsed(which: ResizeRegion): boolean {
...
export function usePanelResize(options: UsePanelResizeOptions = {}) {
  ...
  const centerTopH = ref<number | null>(DEFAULTS.centerTopH);
  const terminalRatio = ref(DEFAULTS.terminalRatio);
  // 拖拽中状态（驱动 divider 视觉态）
  const resizing = ref<ResizeRegion | null>(null); // 'sidebar' | 'right' | 'center-row' | null
   ...
   * @param event pointer 事件（divider 的 pointerdown）
   * @param which 拖拽区域
   */
  ...
  function handleResizeKeydown(event: KeyboardEvent, which: ResizeRegion) {
