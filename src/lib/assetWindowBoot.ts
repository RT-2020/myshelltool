...
import { readHandoffData } from './sessionHandoff';

/** boot 所需的最小 workbench store 形状（App.vue 传入完整 store，结构子集协变兼容）。 */
export interface AssetWindowBootStore {
  assets: Array<{ id?: string } | null | undefined>;
  announce(message: string, options?: { level?: string }): void | Promise<void>;
  selectAsset(id: string, connect?: boolean): void;
  selectedAssetId: string | null;
  connectSelected(): void | Promise<void>;
}

/** adopt 分支所需的最小 sessions store 形状（方法双变，真实 store 协变兼容）。 */
interface BootSessionsStore {
  sessions: Array<{ sessionId?: string }>;
  adoptSession(data: unknown): Promise<boolean>;
}

export interface BootAssetWindowOptions {
  assetId?: string;
  adoptSessionId?: string;
}

export async function bootAssetWindow(store: AssetWindowBootStore, { assetId, adoptSessionId }: BootAssetWindowOptions = {}): Promise<void> {
  ...
  if (adoptSessionId) {
    ...
    if (data) {
      ...
      const sessionsStore: BootSessionsStore = useSessionsStore();
