...
// 对齐：名称左对齐（文本主列，省略号收尾）；其余元数据列一律居中（权限/属主组/大小/类型/修改时间）。
...
const columns = [
  ...
  { key: 'owner', label: '属主/组', className: 'col-owner col-sort col-sort--center' },
...
// 窄栏（双栏模式每栏约 400px）：去掉权限/属主组列并换 4 列模板，否则名称列被元数据列挤没。
// 阈值 660px = 含权限+属主组的固定列约 492px + 名称列约 180px；容器是 FileColumn 的 .file-column。
// 选择器带回 .col-header 前缀：同组件的 .col-sort{display:inline-flex} 与它是同级
// （0,1,0），跨组件打包顺序不保证，必须靠特异性压过，否则隐藏列后表头按钮会掉进隐式列。
@container (max-width: 660px) {
  .col-header .col-perm,
  .col-header .col-owner {
    display: none;
  }
