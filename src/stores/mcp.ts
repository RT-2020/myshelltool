 ...
 * @param url  MCP HTTP endpoint（如 http://127.0.0.1:41235/mcp）
 */
function buildConfigSnippet(url?: string) {
...
/** mcp store 实际消费的 workbench bridge 最小结构（modal setter / announce）。 */
interface McpWorkbenchBridge {
...
export const useMcpStore = defineStore('mcp', () => {
  ...
  const approvalPrompt = ref<McpApprovalPrompt | null>(null);
  // 监听句柄（dispose 时调用）。模块作用域而非 store state——它不是响应式数据。
  let approvalUnlisten: TauriUnlistenFn | null = null;
  ...
  let workbenchBridge: McpWorkbenchBridge | null = null;
  function wb(): McpWorkbenchBridge {
  ...
  async function refresh() {
    ...
    loading.value = true;
    try {
      status.value = await invokeBackend<McpStatusResult>('mcp_status');
    } catch (error) {
  ...
  async function ensureApprovalListener() {
    ...
    try {
      approvalUnlisten = await listenBackendEvent(MCP_APPROVAL_EVENT, event => {
        approvalPrompt.value = event.payload as McpApprovalPrompt;
   ...
   * @param requestId  审批请求 id（来自 approvalPrompt.request_id）
   * @param accepted  用户是否确认执行
   */
  async function resolveMcpApproval(requestId: string, accepted: boolean) {
    try {
   ...
   * @param level  'minimal' | 'strict'
   */
  async function setMcpInterceptLevel(level: string) {
    const label = level === 'strict' ? '全部需确认' : '仅拦截毁灭性命令';
    try {
      const config = await invokeBackend<McpConfigResult>('mcp_set_config', { level });
  ...
  function attachWorkbench(bridge: McpWorkbenchBridge) {
