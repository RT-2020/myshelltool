...
export const useAssetsStore = defineStore('assets', () => {
  ...
  // ============================================================
  const assetSource = ref<AssetListResult>({ source: 'loading', count: 0 });
  const assets = ref<NormalizedConnectionAsset[]>([]);
  ...
  const declaredGroups = ref<string[]>([]);

  // ============================================================
  // 跨 store 桥接（lazy）
  // ============================================================
  let workbenchBridge: AssetsWorkbenchBridge | null = null;
  function attachWorkbench(store: AssetsWorkbenchBridge) {
    workbenchBridge = store;
  }
  function wb(): AssetsWorkbenchBridge {
    if (!workbenchBridge) {
  ...
  // Actions
  // ============================================================
  function selectAsset(id: string, announceSelection = true) {
  ...
  function credentialIdFor(assetId: string, kind: string) {
    return `${assetId}:${kind}`;
  }
  ...
  // ------------------------------------------------------------
  async function duplicateAsset(asset: NormalizedConnectionAsset) {
    ...
    // 副本不带凭据（credential 三字段显式置空）——提示里说清，避免用户
    // 拿副本直连才发现没有密码/私钥（真机验收 2026-09-23 的困惑反馈）。
    announce('已复制连接资产：' + asset.name + ' (副本，不含密码/私钥等凭据，连接前请编辑补充)');
  }

  // ------------------------------------------------------------
  // 移动单个资产到目标分组（单条，复用 saveAsset，无需新命令）。
  // ------------------------------------------------------------
  async function moveAsset(id: string, newGroup: string) {
  ...
  // ------------------------------------------------------------
  async function reorderGroups(orderedPaths?: string[] | null) {
    ...
    try {
      const result = await invokeBackend<AssetListResult>('reorder_asset_groups', { paths: next });
      declaredGroups.value = result.groups || [];
      maybeAutoPush();
    } catch (error) {
  ...
  async function saveToken(secret: string) {
    if (!secret.trim()) {
      announce('token 不能为空');
      return false;
    }
    await invokeBackend('save_credential', { id: 'github-pat', secret });
    const status = await invokeBackend<CredentialStatusResult>('get_credential_status', { id: 'github-pat' });
