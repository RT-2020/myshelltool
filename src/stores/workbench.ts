...
export const useWorkbenchStore = defineStore('workbench', () => {
  ...
  // ============================================================
  async function initialize(options: { mode?: string } = {}) {
    ...
    try {
      const [status, assetResult, credential, tunnelResult] = await Promise.all([
        invokeBackend<BackendStatusState>('backend_status'),
        invokeBackend<AssetListResult>('list_connection_assets'),
        invokeBackend<CredentialStatusResult>('get_credential_status', { id: 'github-pat' }).catch(() => ({ exists: false })),
        invokeBackend<Record<string, unknown>[]>('tunnel_list').catch(() => [])
      ]);
    ...
    //    仅让用户切到 files tab 时即可看到内容。切 tab 时 ui 的 setTab('files')
...
export function normalizeStatus(status: string | null | undefined): { label: string; dotClass: string } {
